...
/* =========================================================
   DEBUG KEYS — ver LocalStorage sin consola (temporal)
   Pegar al FINAL de app.js
========================================================= */
(() => {
  if (document.getElementById('fmKeysFab')) return;

  const fab = document.createElement('button');
  fab.id = 'fmKeysFab';
  fab.type = 'button';
  fab.textContent = '🧾 KEYS';
  fab.style.cssText = "position:fixed;left:12px;bottom:12px;z-index:999999;background:#111;color:#fff;border:1px solid #fff;border-radius:14px;padding:10px 12px;font:700 13px system-ui;box-shadow:0 10px 24px rgba(0,0,0,.25)";
  document.body.appendChild(fab);

  const modal = document.createElement('div');
  modal.id = 'fmKeysModal';
  modal.style.cssText = "position:fixed;inset:0;z-index:9999999;display:none;background:rgba(0,0,0,.55)";
  modal.innerHTML = `
    <div style="position:absolute;left:12px;right:12px;top:12px;bottom:12px;background:#fff;border:1px solid #111;border-radius:16px;padding:12px;display:flex;flex-direction:column;gap:10px">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:10px">
        <b style="font:800 14px system-ui">LocalStorage KEYS (Factu Miral)</b>
        <button id="fmKeysClose" style="border:1px solid #111;background:#fff;border-radius:12px;padding:8px 10px;font:800 13px system-ui">Cerrar</button>
      </div>
      <div style="display:flex;gap:10px;flex-wrap:wrap">
        <button id="fmKeysCopy" style="border:1px solid #111;background:#111;color:#fff;border-radius:12px;padding:10px 12px;font:800 13px system-ui">Copiar</button>
        <button id="fmKeysRefresh" style="border:1px solid #111;background:#fff;border-radius:12px;padding:10px 12px;font:800 13px system-ui">Actualizar</button>
      </div>
      <pre id="fmKeysPre" style="flex:1;overflow:auto;border:1px solid rgba(0,0,0,.2);border-radius:14px;padding:10px;background:#f7f7f7;font:12px ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, 'Liberation Mono', monospace;white-space:pre-wrap"></pre>
      <div style="font:12px system-ui;opacity:.7">Tip: busca aquí cuál key tiene tus facturas/clientes/productos.</div>
    </div>
  `;
  document.body.appendChild(modal);

  const pre = modal.querySelector('#fmKeysPre');
