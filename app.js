...
function fillPrint(lines, totals, temp=null, extra=null){
  $('#p-num').textContent = extra?.numero || '(Sin guardar)';
  $('#p-fecha').textContent = (extra?new Date(extra.fecha):new Date()).toLocaleString();
  ...
  $('#p-estado').textContent = extra?.estado || $('#estado')?.value || 'Impagada';
  $('#p-metodo').textContent = extra?.metodo || $('#metodoPago')?.value || 'Efectivo';
  $('#p-obs').textContent = extra?.obs || ($('#observaciones')?.value||'—');
