...
/* =========================================================
   ADDON 1/3 — BACKUP/RESTORE + AUTO-BACKUP (SEGURO)
   - Exporta/Importa JSON de LocalStorage factumiral_*
   - Auto-backup diario (3 copias internas)
   ✅ Pegar al FINAL de app.js
========================================================= */
(() => {
  'use strict';

  const PREFIX = 'factumiral_';
  const K_AUTOBACK = PREFIX + 'autobackups_v1';
  ...
  function getKeys(){
    ...
    // evitamos cosas temporales si existieran
    return out.filter(k =>
      !k.includes('last_pdf') &&
      !k.includes('cloud_session')
    );
  }
  ...
  function exportBackup(){
    ...
    const name =
      `factu-miral-backup-${d.getFullYear()}${String(d.getMonth()+1).padStart(2,'0')}${String(d.getDate()).padStart(2,'0')}` +
  ...
  async function importBackupFromFile(file){
    ...
    // Restaurar SOLO keys factumiral_*
  ...
  function autoBackupDaily(){
    try{
      const pack = JSON.parse(localStorage.getItem(K_AUTOBACK) || '{"last":0,"list":[]}');
      const last = pack.last || 0;
      const oneDay = 24*60*60*1000;

      if (now() - last < oneDay) return;

      const snap = buildBackupPayload(); // incluye data completa
      const list = Array.isArray(pack.list) ? pack.list : [];
      list.unshift({ ts: snap.ts, data: snap.data });

      while (list.length > 3) list.pop(); // 3 copias

      localStorage.setItem(K_AUTOBACK, JSON.stringify({ last: now(), list }));
    } catch {}
  }

  function restoreAutoBackup(idx){
    try{
      const pack = JSON.parse(localStorage.getItem(K_AUTOBACK) || '{"list":[]}');
      const list = Array.isArray(pack.list) ? pack.list : [];
      const snap = list[idx];

      if (!snap?.data) {
        alert('No existe esa copia.');
        return;
      }
      ...
      alert('Error restaurando copia.');
    }
  }

  // Exponer funciones para el ADDON 2/3 (menú PRO)
  window.__FM_BACKUP__ = {
    exportBackup,
    importBackupFromFile,
    restoreAutoBackup
  };

  // Ejecutar auto-backup (seguro)
  autoBackupDaily();
})();
/* =========================================================
   ADDON 2/3 — BARRA MÓVIL (ACCIONES RÁPIDAS)
   - Guardar / PDF / WhatsApp / Nueva / Duplicar / Buscar
   - No toca tu core: hace click en tus botones si existen
   ✅ Pegar al FINAL de app.js (debajo del Addon 1)
========================================================= */
(() => {
  'use strict';
  ...
  function openTabFacturas(){
    // intenta ir a tab Facturas (si existe botón)
    const btn = document.querySelector('[data-tab="tabFacturas"], #tabBtnFacturas, .tab[data-tab="tabFacturas"]');
    if (btn) btn.click();
  }

  function openSearch(){
    // si existe input de búsqueda en listado facturas, enfocarlo
    openTabFacturas();
    setTimeout(() => {
      const inp = document.querySelector('#factSearch, #buscaFacturas, input[placeholder*="Buscar"], input[aria-label*="Buscar"]');
      if (inp) inp.focus();
    }, 120);
  }

  function injectCSS(){
    if ($('#fmActionBarCss')) return;
    const st = document.createElement('style');
    st.id = 'fmActionBarCss';
    st.textContent = `
      .fmActionBar{
      ...
      .fmActionBar .fmABtn span{ font-size:18px; }
      .fmActionBar .fmABtn:active{ transform:translateY(1px); }

      /* solo móvil */
      @media (max-width: 900px){
        .fmActionBar{ display:flex; }
        body{ padding-bottom: 92px !important; } /* deja espacio para no tapar */
      }
      /* si hay modal PDF abierto, bajar prioridad visual */
      .fmActionBar.isDim{ opacity:.55; }
    `;
    document.head.appendChild(st);
  }

  function mount(){
    if ($('#fmActionBar')) return;

    const bar = document.createElement('div');
    bar.className = 'fmActionBar';
    bar.id = 'fmActionBar';
    ...
    const mk = (icon, label, fn) => {
      ...
      b.type = 'button';
      b.className = 'fmABtn';
      b.innerHTML = `<span>${icon}</span><div>${label}</div>`;
      b.addEventListener('click', fn);
      return b;
    };

    bar.appendChild(mk('💾','Guardar', () => clickIf('#btnGuardarFactura')));
    bar.appendChild(mk('📄','PDF',     () => clickIf('#btnPdf')));
    bar.appendChild(mk('🟢','Whats',   () => clickIf('#btnWhatsApp')));
    bar.appendChild(mk('🆕','Nueva',   () => clickIf('#btnNuevaFactura')));
    bar.appendChild(mk('📑','Duplicar',() => clickIf('#btnDuplicarFactura')));
    bar.appendChild(mk('🔎','Buscar',  () => openSearch()));

    document.body.appendChild(bar);

    // si aparece modal PDF, atenúa barra (no molesta)
    const pdfModal = document.querySelector('#pdfModal');
    if (pdfModal) {
      const obs = new MutationObserver(() => {
        const open = !pdfModal.classList.contains('is-hidden');
        bar.classList.toggle('isDim', open);
      });
      obs.observe(pdfModal, { attributes:true, attributeFilter:['class'] });
    }
  }
...
/* =========================================================
   ADDON 3/3 — DIAG + FIX DECIMALES (PRECIO EN GRID)
   - Toggle DIAG: triple toque fuera del grid (o manten 0.7s en ⚙️ si existe)
   - Fix decimales: fuerza Precio a text + inputmode decimal y restaura "." si el core lo borra
   ✅ Pegar al FINAL de app.js
========================================================= */
(() => {
  'use strict';
  ...
  function ensureDiag(){
    ...
    diagBox.style.cssText = `
      position:fixed; left:12px; right:12px; bottom:96px; z-index:99999;
      border:1px solid #111; border-radius:14px; background:#fff;
      box-shadow:0 10px 30px rgba(0,0,0,.20);
      padding:10px; font:12px system-ui; color:#000; display:none;
  ...
  function renderDiag(){
    ...
    const info = (!el || el === document.body) ? '—' : `
tag=${el.tagName}
id=${el.id || '—'}
...
`;
    ...
    diagBox.textContent = `DIAG ${DIAG ? '(ON)' : '(OFF)'}\n${info}`.trim();
  ...
  function toggleDiag(){
    DIAG = !DIAG;
    ensureDiag();
    diagBox.style.display = DIAG ? 'block' : 'none';
    if (DIAG) renderDiag();
  }

  document.addEventListener('keydown', (e) => { lastKey = e.key; if (DIAG) renderDiag(); }, true);
  document.addEventListener('input', () => { if (DIAG) renderDiag(); }, true);
  document.addEventListener('focusin', () => { if (DIAG) renderDiag(); }, true);

  // Toggle: triple click fuera del grid
  (function bindTriple(){
    let c = 0, t = null;
    document.addEventListener('click', (e) => {
      if (e.target.closest('#gridBody')) return;
      c++;
      clearTimeout(t);
      t = setTimeout(()=>{ c=0; }, 650);
      if (c >= 3) { c=0; toggleDiag(); }
    }, true);
  })();

  // Toggle por long-press en ⚙️ si existe
  (function bindLongPress(){
    const btn = document.querySelector('#fmFab .fmFabBtn'); // si usas FAB
    if (!btn || btn.__lp) return;
    btn.__lp = true;
    let tt = null;
    btn.addEventListener('touchstart', () => {
      tt = setTimeout(()=> toggleDiag(), 700);
    }, { passive:true });
    btn.addEventListener('touchend', () => { if (tt) clearTimeout(tt); tt=null; }, { passive:true });
  })();

  /* ---------- FIX DECIMALES PRECIO ---------- */

  function looksLikePrecioInput(el){
    if (!el || el.tagName !== 'INPUT') return false;

    // lo limitamos al GRID de factura para no tocar otros inputs
    const row = el.closest('#gridBody .gridRow, #gridBody, .gridRow');
    if (!row) return false;
    ...
    // señales típicas
    if (cls.includes('precio') || ph.includes('precio') || nm.includes('precio') || df === 'precio' || dc === 'precio') return true;

    // fallback seguro: si hay 1 input con placeholder Precio en la fila
    return false;
  }

  function forceDecimal(el){
    if (!el || el.__fmDec) return;
    el.__fmDec = true;

    // si el core insiste en number/numeric, lo forzamos y vigilamos
    const apply = () => {
      try { if (el.type !== 'text') el.type = 'text'; } catch {}
      el.setAttribute('inputmode','decimal');
      el.setAttribute('pattern','[0-9]*[\\.,]?[0-9]*');
      el.setAttribute('autocomplete','off');
      el.setAttribute('autocorrect','off');
    ...
    new MutationObserver(apply).observe(el, { attributes:true, attributeFilter:['type','inputmode','pattern'] });

    // sanitiza a formato interno con punto (para que Number() lo entienda)
    const sanitize = (raw) => {
      let v = String(raw ?? '').replace(/\s+/g,'');
      v = v.replace(/[^\d.,-]/g,'');
      v = v.replaceAll(',', '.');
    ...
    // si core borra el ".", lo reponemos al final del tick
    let wanted = null;
    ...
    // captura '.'/',' aunque el core lo bloquee
    const handleDotKey = (e) => {
      ...
      e.stopImmediatePropagation();
      insertDot();
      wanted = sanitize(el.value);
      setTimeout(() => {
        if (!el.isConnected) return;
        // si alguien lo borró, lo restauramos
        el.value = wanted;
        el.dispatchEvent(new Event('input', { bubbles:true }));
      }, 0);
    };

    el.addEventListener('keydown', handleDotKey, true);
    el.addEventListener('keypress', handleDotKey, true);
    ...
    el.addEventListener('input', () => {
      ...
      // asegurar al final del tick (por si el core lo vuelve a cambiar)
      setTimeout(() => {
        if (!el.isConnected) return;
        if (sanitize(el.value) !== wanted) el.value = wanted;
      }, 0);
    }, true);
  }

  function scanGrid(){
    // Caso normal: clase .precio
    $$('#gridBody .precio').forEach(forceDecimal);

    // Caso fallback: detectores por placeholder/name/data
    $$('#gridBody input').forEach(inp => {
      const ph = (inp.getAttribute('placeholder')||'').toLowerCase();
      const nm = (inp.getAttribute('name')||'').toLowerCase();
      const cls = (inp.className||'').toLowerCase();
      const df = (inp.getAttribute('data-field')||'').toLowerCase();
      const dc = (inp.getAttribute('data-col')||'').toLowerCase();

      const isPrecio =
        cls.includes('precio') || ph.includes('precio') || nm.includes('precio') || df==='precio' || dc==='precio';

      if (isPrecio) forceDecimal(inp);
    });
  }

  // aplicar ya + a filas nuevas
  function boot(){
    scanGrid();
    const grid = $('#gridBody');
    if (grid && !grid.__fmDecObs){
      grid.__fmDecObs = true;
